...
public class MeltDreamEnergyVaryMobEffect extends InstantenousMobEffect {
    private final boolean isIncrease;
	public MeltDreamEnergyVaryMobEffect(boolean isIncrease) {
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        if(entity instanceof ServerPlayer pl)
        {
            if(isIncrease)
            {
                MeltDreamEnergyCapability.addPlayerMeltDreamEnergy(pl,(amplifier & 0xff) + 1);
            }
            else
            {
                MeltDreamEnergyCapability.addPlayerMeltDreamEnergy(pl,- (amplifier & 0xff) - 1);
